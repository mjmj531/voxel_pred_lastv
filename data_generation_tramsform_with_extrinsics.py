...
def load_saved_camera_extrinsics(scene_id, index, base_path='cam_extrinsics_param_1111'):
    """
    从指定路径加载保存的外参矩阵。
    :param scene_id: 场景的唯一标识符
    :param index: 外参矩阵的编号
    :param base_path: 外参矩阵的基础路径
    :return: 加载的外参矩阵 (torch.Tensor)
    """
    file_path = os.path.join(base_path, scene_id, f'extrinsics_{index}.json')
...
def save_ray_points_camera_nv(image_width, image_height, intrinsics, file_path, step_size, depth=2.0):
    ...
    """
    # 计算每条光线的点数
    num_points = int(depth / step_size)
    ...
    intrinsics_inv = torch.inverse(intrinsics)
    ray_direction_camera = normalize_vector_all(pixel_to_camera_coords_all(pixels, intrinsics_inv))
    print("ray_direction_camera.shape: ", ray_direction_camera.shape)
    ...
    # 保存点云数据
    np.save(file_path, ray_points_camera.cpu().numpy())
    # np.save(file_path.replace('.npy', '_test.npy'), ray_points_camera_test.cpu().numpy())
...
def save_npy_to_ply(ray_points_camera, mask, save_path, filename):
    ...
    :param filename: str, 保存的文件名
    """
    ray_points_camera = ray_points_camera.cpu().numpy()
    _, max_steps, _ = ray_points_camera.shape
    # 确保点云数据在CPU上并转换为numpy格式
    ply_mask = mask.cpu().numpy()
    ply_mask = ply_mask.reshape(-1, max_steps)
    if ply_mask.size == 0:
        print("Warning: Point cloud data is empty.")
        return
...
# 处理单个场景中的128个有效外参矩阵
def save_gt_input_from_extrinsics(occupancy_grid, num, intrinsics, image_width, image_height, data_folder, scene, step_size):
    # max_steps = int(deep / step_size)
    ray_points_camera, max_steps = save_ray_points_camera_nv(image_width, image_height, intrinsics, 'ray_points_camera_1111.npy', step_size=step_size)
    cnt = 0
    while cnt < num:
        extrinsics = load_saved_camera_extrinsics(scene, cnt, 'cam_extrinsics_param_1111')
        ...
        if input_info is not None and ground_truth is not None:
            ...
            # 保存点云
            gt_ply_folder = 'gt_ply_1111'
            os.makedirs(gt_ply_folder, exist_ok=True)
            input_ply_folder = 'input_ply_1111'
            ...
            # save_pointcloud_to_ply(input_info, input_ply_folder, f"{scene}_{file_suffix}_input_info.ply")
            # save_pointcloud_to_ply(ground_truth, gt_ply_folder, f"{scene}_{file_suffix}_ground_truth.ply")
...
def process_all_scenes(data_folder, scene_folder, step_size = 0.04, num = 128):
    ...
    for scene_file in files:
        if scene_file.endswith('.npy'):
            ...
            # # 设置相机外参文件夹路径
            # extrinsics_folder = os.path.join('cam_extrinsics_param_1111', scene)
            # os.makedirs(extrinsics_folder, exist_ok=True)

            # 生成并保存有效的外参矩阵
            save_gt_input_from_extrinsics(
                occupancy_grid, num, intrinsics, image_width, image_height,
                data_folder, scene, step_size=step_size
            )
